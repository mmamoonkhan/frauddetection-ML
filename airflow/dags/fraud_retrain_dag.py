...
def check_data_drift(**ctx):
    ...
    for feature in ["amount", "distance_from_home", "num_transactions_last_24h"]:
        # Compare the new data directly against the stored baseline sample.
        # Using a real empirical sample avoids the pitfall of approximating
        # exponential/Poisson distributions with a Gaussian (which would
        # cause the KS test to always flag drift due to shape mismatch).
        ref_sample = np.array(baseline[feature]["sample"])
...
def save_to_registry(**ctx):
    ...
    # copyfile instead of copy: only copies content, not file permissions.
    # shutil.copy would also try chmod, which fails when the destination was
    # created by a different user (e.g. host user vs airflow user in Docker).
    ...
    # ── Update baseline for next drift detection ──────────────────────────
    df = pd.read_csv(PROCESSED_PATH)
    baseline = {}
    for feature in ["amount", "distance_from_home", "num_transactions_last_24h"]:
        baseline[feature] = {
            "sample": df[feature].dropna().tolist()
        }
    with open(BASELINE_PATH, "w") as f:
        json.dump(baseline, f, indent=2)
    log.info(f"Baseline sample saved → {BASELINE_PATH} ({len(df)} rows)")
